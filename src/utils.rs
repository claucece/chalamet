...
pub mod format {
  ...
  pub fn sha256_into_u64_sized(
    input: &[u8],
  ) -> Result<[u64; 4], ErrorUnexpectedInputSize> {
    let mut h = Sha256::new();
    h.update(input);
    let digest = h.finalize();
    if digest.len() != 32 {
      return Err(ErrorUnexpectedInputSize::new(format!(
        "Digest should be 32 bytes, but instead it is {} bytes long",
        digest.len()
      )));
    }
    let mut key = [0u64; 4];
    for i in 0..4 {
      let bv = digest[8 * i..8 * (i + 1)].to_vec();
      let bytes: [u8; 8] = match bv.try_into() {
        Ok(b) => b,
        Err(e) => {
          return Err(ErrorUnexpectedInputSize::new(format!(
            "Unexpected vector size: {:?}",
